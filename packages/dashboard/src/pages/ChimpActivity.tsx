...
// ─── Opencode thought rendering ────────────────────────────────────────

function renderOpencodePart(part: Opencode.Part, i: number) {
  switch (part.type) {
    case "text":
      return part.text ? (
        <div
    ...
    case "tool":
      return renderOpencodeToolPart(part, i);
...
function renderOpencodeToolPart(part: Opencode.ToolPart, i: number) {
  const { state } = part;
  const statusColor = {
...
function renderOpencodeThought(event: Opencode.Event) {
  switch (event.type) {
    ...
    case "message.part.updated":
      return renderOpencodePart(event.properties.part, 0);
    ...
    switch (data.type) {
      ...
      case "thought": {
        ...
        if (data.brain === "opencode") {
          return renderOpencodeThought(data.event as Opencode.Event);
        }
